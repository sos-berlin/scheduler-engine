...
trait OrderEntityConverter extends ObjectEntityConverter[OrderPersistent, OrderKey, OrderEntity] {
  ...
  final def toObject(e: OrderEntity) = OrderPersistent(
      jobChainPath = new JobChainPath(AbsolutePath.of(e.jobChainPath)),
      ...
      priority = e.priority,
      ordering = e.ordering,
      stateOption = Option(e.state) map OrderState,
      initialStateOption = Option(e.initialState) map OrderState,
