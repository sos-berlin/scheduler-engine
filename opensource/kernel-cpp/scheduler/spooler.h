                 ...
                 javabridge::has_proxy<Spooler>
{
  ...
  //int                         priority_max                () const                            { return _priority_max; }
    ...
    string                      execute_xml_with_security_level (const string& xml, const string& security_level);
    string                      execute_xml_with_security_level (const string& xml, Security::Level security_level);
  ...
  //void                        create_window               ();
    ...
    // Prozesse
    void                        register_process_handle     ( Process_handle );                 // F�r abort_immediately()
    void                        unregister_process_handle   ( Process_handle );                 // F�r abort_immediately()
    void                        register_pid                ( int, bool is_process_group = false ); // F�r abort_immediately()
    void                        unregister_pid              ( int );                            // F�r abort_immediately()
  ...
  public:
    ...
    bool                       _log_directory_as_option_set;// -log-dir= als Option gesetzt, �berschreibt Angabe in spooler.xml
    string                     _log_filename;
    bool                       _log_to_stderr;              // Zus�tzlich nach stdout schreiben
    ...
    bool                       _include_path_as_option_set; // -include-path= als Option gesetzt, �berschreibt Angabe in spooler.xml
    string                     _temp_dir;
    string                     _spooler_param;              // -param= Parameter f�r Skripten
    bool                       _spooler_param_as_option_set;// -param= als Option gesetzt, �berschreibt Angabe in spooler.xml
  //int                        _priority_max;               // <config priority_max=...>
    ...
    bool                       _mail_on_warning;            // F�r Job-Protokolle
    bool                       _mail_on_error;              // F�r Job-Protokolle
    int                        _mail_on_process;            // F�r Job-Protokolle
    bool                       _mail_on_success;            // F�r Job-Protokolle
    First_and_last             _mail_on_delay_after_error;  // F�r Job-Protokolle
    ...
    bool                       _interactive;                // Kommandos �ber stdin zulassen
    bool                       _manual;
  //string                     _job_name;                   // Bei manuellem Betrieb
    ...
    file::File_path            _central_configuration_directory;         // F�r den Supervisor zur Replikation
    ...
    Process_handle             _process_handles[ max_processes ];   // F�r abort_immediately(), mutex-frei alle abh�ngigen Prozesse
    struct Killpid { int _pid; bool _is_process_group; };
    Killpid                    _pids[ max_processes ];              // F�r abort_immediately(), mutex-frei alle Task.add_pid(), Subprozesse der Tasks
    bool                       _are_all_tasks_killed;

    schedule::Holidays         _holidays;                   // Feiertage f�r alle Jobs

    State_changed_handler      _state_changed_handler;      // Callback f�r NT-Dienst SetServiceStatus()

    xml::Document_ptr          _config_document_to_load;    // F�r cmd_load_config(), das Dokument zu _config_element_to_load
    xml::Element_ptr           _config_element_to_load;     // F�r cmd_load_config()
    string                     _config_source_filename;     // F�r cmd_load_config(), der Dateiname der Quelle
    ...
    Time                       _last_wait_until;            // F�r <show_state>
    Time                       _last_resume_at;             // F�r <show_state>
    ...
    time_t                     _termination_gmtimeout_at;   // F�r sc_terminate und sc_terminate_with_restart
    ...
    Variable_set_map           _variable_set_map;           // _variable_set_map[""] = _environment; f�r <params>, Com_variable_set::set_dom()
