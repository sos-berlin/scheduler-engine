...
struct Dependant                // Abh�ngig von anderen File_based (Requisite)
{
                                Dependant                   ();
    virtual                    ~Dependant                   ();
  ...
  private:
    ...
    stdext::hash_set<Dependant*> _accompanying_dependants;    // missing_requistes() liefert auch die Vermissten dieser Dependants, z.B. Schedule_use f�r Job
};
                    ...
                    zschimmer::Has_addref_release
{
    enum State
    {
        ...
        s_loaded,               // Mit Daten gef�llt: bei Jobs die Task-Warteschlange, bei Jobketten die Auftragswarteschlangen
        s_incomplete,           // Requisite fehlt (beim Versuch zu aktivieren)
        s_active,
        s_closed
    };

    // Zutun: Genauer kl�ren, was die Zust�nde bedeuten, f�r jede Klasse, besonders Fehlerf�lle (einschlie�lich fehlender Requisiten)
    // s_incomplete vielleicht automatisch setzen, nach on_requisite_loaded() wieder versuchen zu aktivieren
    
    // Diese generischen Zust�nde mit den Zust�nden der Objekte (Job, Order) unter einem Hut bringen. 
    ...
    { 
        rm_standard, 
        rm_base_file_too,
        rm_temporary,           // Objekt wird nur vorr�bergehend gel�scht, weil es ersetzt oder weil der Scheduler beendet wird.
    };
    ...
    Absolute_path               path                        () const;                               //2008-07-09 K�nnte path_or_slash() hei�en?
    Absolute_path               path_or_empty               () const;                               //2008-07-09 K�nnte path() hei�en?
  ...
  //void                    set_defined                     ();                                     // F�r Objekte, die kein XML brauchen
...
//-------------------------------------------------------------------------------------------Folder
//
// Ein Ordner (Folder) enth�lt alle Objekte. 
                ...
                Object
{
  ...
  //void                        remove_all_file_baseds      ();
    ...
    Absolute_path               make_path                   ( const string& name );                 // H�ngt den Ordernamen voran
...
// F�r jeden dateibasierten Typ (File_based) gibt es genau ein File_based_subsystem
...
struct file_based_subsystem : File_based_subsystem
{
    ...
    virtual vector<FILE_BASED*> ordered_file_baseds()
    {
        ...
        //Wir m�ssen nicht ordnen und ordering_is_less() betrachtet nicht den Pfad: sort( result, result.begin(), result.end(), File_based::ordering_is_less );
  ...
  private:
    ...
    void add_file_based( File_based* file_based )                   // Nur f�r Typed_folder::add_file_based()
    {
    ...
    void remove_file_based( File_based* file_based )                // Nur f�r Typed_folder::remove_file_based()
    {
        assert( file_based_or_null( file_based->path() ) );
    ...
    void replace_file_based( File_based* old_file_based, File_based* new_file_based )       // Nur f�r Typed_folder::replace_file_based()
    {
