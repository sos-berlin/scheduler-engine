...
// K�nnte als Unterklasse von Web_service_operation implementiert werden
        ...
        if( http_request->_http_cmd == "GET" )
        {
            ...
            if( string_ends_with( path, "?" ) )
            {
                ...
                if( string_ends_with( path, "/show_config?" ) )
                {
                    ...
                    throw http::Http_exception( http::status_404_bad_request, "Ung�ltiger URL-Pfad: " + path );
            }
            else
            {
...
xml::Document_ptr Command_processor::dom_from_xml( const string& xml_text )
{
    ...
    if( !ok )
    {
        ...
        _spooler->log()->error( text );       // Log ist m�glicherweise noch nicht ge�ffnet
