            ...
            if( _spooler->_order_history_with_log )
            {
                ...
                if( log_text != "" )
                {
                    try 
                    {
                        ...
                        blob_file.put( log()->truncate_head(log_text) );
                        blob_file.close();
                    }
                    catch( exception& x ) 
                    { 
...
xml::Element_ptr Order::dom_element( const xml::Document_ptr& dom_document, const Show_what& show_what, const string* log ) 
{
    ...
    if( show_what.is_set( show_log )  ||  show_what.is_set( show_for_database_only ) )
    {
        ...
        if( log  &&  show_what.is_set( show_log ) ) result.append_new_text_element( "log", Prefix_log::truncate_head(*log,1024) );     // Protokoll aus der Datenbank
        else
