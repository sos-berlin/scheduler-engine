...
//----------------------------------------------------------------------------------------Has_proxy
// Legt bei Bedarf einen CppProxy an (vom Java-C++-Generator erzeugt) und und l�st ihn am Ende wieder.
// Der CppProxy h�lt nicht dieses C++-Object. Wenn das C++-Objekt zerst�rt wird, wird die Verbindung vom
// CppProxy zum C++-Objekt gel�st und die n�chste Verwendung des CppProxy liefert den Fehler Z-JAVA-111.
...
private:
    ...
    Has_proxy* const           _sister_context_proxy;       // Kontext f�r die Java-Schwester oder NULL
    Global_jobject2            _java_sister;                // Manche C++-Klassen haben neben dem C++-Proxy eine Java-Schwester, eine von C++ unabh�ngige Klasse
};
...
//----------------------------------------------------------------------------------proxy_jobject<>
// Typsicheres jobject, f�r Parameter�bergabe von C++ an Java, 
// damit nicht Global_jobject2 mit NewGlobalRef() und DeleteGlobalRef() verwendet werden muss.

template<class PROXY>   // PROXY: Vom C++/Java-Generator erzeugte Klasse f�r Java-Proxy, z.B. SpoolerC
