...
import com.sos.scheduler.engine.tests.jira.js795.WebServiceFunctions.noCache
...
class CommandResource @Inject()(xmlCommandExecutor: SchedulerXmlCommandExecutor) {
  ...
  private def executeCommand(command: String) = {
    val resultXml = xmlCommandExecutor.uncheckedExecuteXml(command) + CommandServlet.testString
