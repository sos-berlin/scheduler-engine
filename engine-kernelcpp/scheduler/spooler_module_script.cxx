...
void Java_module_script_instance::init()
{
    ...
    string java_class_name = _module->_is_monitor? java_adapter_monitor() : java_adapter_job();

    Module_instance::init();

    _java_class = env.find_class(java_class_name);
    jmethodID method_id = java_method_id( "<init>(Ljava/lang/String;Ljava/lang/String;)V" );   // Konstruktor
    if (!method_id)  env.throw_java("GetMethodID", java_class_name);
    ...
    if (!_jobject || env->ExceptionCheck())  env.throw_java(java_class_name + " constructor");
