    ...
    for ((jobPath, n) ← Array(JobPath("/invalid-credentials-shell") → 50, JobPath("/invalid-credentials-api") → 3)) {
      s"Unknown credential key $jobPath" in {
        controller.toleratingErrorCodes( _ ⇒ true) {
          val taskIds = for (_ ← 1 to n) yield {
            val result = startJob(jobPath)
            result.closed await TestTimeout  // Not result.ended, because there is no TaskEndedEvent
            result.taskId
          }
          // Now, logString must be fetched from database
          for (taskId ← taskIds) {
            //assert(taskRun.returnCode == ReturnCode(1))
            assert(taskLog(taskId) contains "Windows Credential Manager does not return an entry named 'NON-EXISTING-CREDENTIALS-KEY': WINDOWS-1168 (CredRead)")
          }
        }
      }
    }
