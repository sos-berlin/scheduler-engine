...
final class JS1642IT extends FreeSpec with ScalaSchedulerTest with SpeedTests {
  ...
  "order" - {
    ...
    "JSON" - {
      ...
      "orderOverviews" in {
        val jsObject = webSchedulerClient.get[JsObject](_.order[OrderOverview]) await TestTimeout
        assert((jsObject.fields("orders").asJsArray map normalizeOrderOverviewJson) == ExpectedOrderOverviewsJsArray)
      }
