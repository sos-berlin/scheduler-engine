...
import com.sos.scheduler.engine.common.scalautil.Closers.implicits.RichClosersHasClose
...
final class DatabaseSubsystem private[kernel](cppProxy: DatabaseC) extends Subsystem with HasCloser {
  ...
  lazy val entityManagerFactory: EntityManagerFactory = {
    try createEntityManagerFactory(persistenceUnitName, entityManagerProperties).closeWithCloser  // closes all EntityManager, too
    catch {
