...
{ 
    THREAD_LOCK( _lock )  
    {
        ...
        if( _end )  _next_time = 0;   // Falls vor set_state() cmd_end() gerufen worden ist. Damit _end ausgef¸hrt wird.
    ...
    try
    {
        ...
        while( loop )
        {
            ...
            switch( _state )
            {
                ...
                {
                    if( take_order( now ) )  set_state( s_running ), loop = true;  // Auftrag da? Dann Task weiterlaufen lassen (Ende der Run_time wird noch gepr¸ft)
