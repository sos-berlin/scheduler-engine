...
//? Type_repr&                  repr                    () const                           { _return *_type_repr_ptr;     }
    ...
    // Binär (kompatibel auf allen rechnern): (persistenz)
  //virtual void                save_value              ( ostream&, const void* ) const;                  // Binäre Ausgabe
  //virtual void                restore_value           ( istream&, void*       ) const;                  // Binäre Eingabe

    // Ein-/ausgabe im fenster:
  //virtual void                show_value              ( Window*, Resource_id, const void*   ) const;    // Anzeige im Fenster
  //virtual void                edit_value              ( Window*, Resource_id, void*         ) const;    // Editieren im Fenster
...
struct Type_fundamental : Type
{
    ...
    //, plain_float
                                ...
                                Type_fundamental        ( Plain_type, int size, Bool is_signed = true );
...
struct Type_struct : Type
{
    enum What
    {
        ...
        readable_with_names
                                ...
                                Type_struct             ( const char* name_ptr, int size, const Member_descr[], int no_of_members );
  ...
  private:
    const int                  _size;
    const int                  _no_of_members;
    const Member_descr*        _member_descr;
};
...
const static int member_descr_unknown_offset = -1;

struct Member_descr : Field_descr
{
                                Member_descr            ( const char* name, const Type*, int offset = member_descr_unknown_offset );
