...
Private Sub Host_Check(Auswertung As Boolean)
    ...
    For i = RecordArray.LowerBound(1) To RecordArray.UpperBound(1)
        ...
        If RecordArray(i, 3) <> "" Then
            RecordArray(i, 3) = str(Val(RecordArray(i, 3)) - 1)
        End If
    Next i
    ...
    For i = RecordArray.LowerBound(1) + 1 To RecordArray.UpperBound(1)
        If RecordArray(i, 0) <> "" Then
            ...
            If Pos <> -1 Then
            If RecordArray(i, 3) = "2" Then
                RecordArray(i, 3) = "3"
            Else
                RecordArray(i, 3) = "1"
            End If
            If RecordArray(Pos, 3) = "2" Then
                RecordArray(Pos, 3) = "3"
            Else
                RecordArray(Pos, 3) = "1"
            End If
...
Private Sub Licence_Check()
    ...
    For i = RecordArray.LowerBound(1) To RecordArray.UpperBound(1)
        RecordArray(i, 2) = "" ' Zurücksetzen
        If Val(RecordArray(i, 3)) > 1 Then
            RecordArray(i, 3) = str(Val(RecordArray(i, 3)) - 2)
        End If
    Next i
    ...
    For i = RecordArray.LowerBound(1) + 1 To RecordArray.UpperBound(1)
        ...
        If Pos = -1 Then
        Else
            If RecordArray(i, 3) = "1" Then
                RecordArray(i, 3) = "3"
            Else
                RecordArray(i, 3) = "2"
            End If
            If RecordArray(Pos, 3) = "1" Then
                RecordArray(Pos, 3) = "3"
            Else
                RecordArray(Pos, 3) = "2"
            End If
...
Private Sub Marker_Auswerten()
    ...
    For i = RecordArray.LowerBound(1) To RecordArray.UpperBound(1)
        ...
        End If
        If RecordArray(i, 3) = "2" And Trim$(RecordArray(i, 0)) <> "" Then
            Speicherbar = False
            RecordArray(i, 2) = "Lizenz doppelt"
        End If
        If RecordArray(i, 3) = "3" Then
            Speicherbar = False
            RecordArray(i, 2) = "Rechnername und Lizenz doppelt"
        End If
        If RecordArray(i, 3) = "" And RecordArray(i, 0) = "" Then
    ...
    End If
    Label1.Caption = str$(RecordArray.UpperBound(1)) + " Einträge, " + str$(FreeCount) + " freie Schlüssel"
...
Private Function Write_Licence_File() As Boolean
    ...
    out = "[licence]" + Zeilenende + "; DO NOT CHANGE: Diese Datei wurde automatisch mit licadm geschrieben." + Zeilenende
    
    For i = RecordArray.LowerBound(1) To RecordArray.UpperBound(1)
        If RecordArray(i, 3) <> "" Then ' irgendwas ist falsch
            MsgBox "Speichern nicht möglich, da noch Inkonsistenzen vorhanden sind!"
...
Static rec As Boolean
    ...
    If Initialized And DataChanged Then
        DataChanged = False
        'TDBGrid1.Update
        If Trim$(RecordArray(LastRow, 0)) = "" And Val(RecordArray(LastRow, 3)) > 1 Then
