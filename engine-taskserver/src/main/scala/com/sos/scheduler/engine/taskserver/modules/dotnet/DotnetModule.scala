...
extends ApiModule {
  ...
  def newMonitorInstance(namedIDispatches: TypedNamedIDispatches, stderrLogLevel: SchedulerLogLevel) =
    factory.newInstance(classOf[sos.spooler.IMonitor_impl], namedIDispatchesToTaskContext(namedIDispatches, stderrLogLevel), arguments.reference)
}
