...
ptr<Web_service_operation> Web_service::new_operation( Http_operation* http_processor )
{
    return Z_NEW( Web_service_operation( this, http_processor, _next_operation_id++ ) );
}
...
Web_service_operation::Web_service_operation( Web_service* ws, Http_operation* ht, int operation_id ) 
: 
    _zero_(this+1), 
    Idispatch_implementation( &class_descriptor ),
...
//-------------------------------------------------------Web_service_operation::process_http__begin

void Web_service_operation::process_http__begin( Http_operation* http_processor )
{
    string response;
...
{
    return _web_service_response->_http_response._ready;
...
ptr<Http_response> Web_service_operation::process_http__end()
{
    ...
    string              response;
    response = process_request__end( http_request->_body, http_request->character_encoding() );
...
string Web_service_operation::process_request__old_style( const string& request_data, const string& character_encoding )
{
    ...
    //if( is_xml )
    {
        ...
        if( !ok )
        {
            ...
            throw Http_operation::Http_exception( Http_operation::code_bad_request );
        }
    }
...
STDMETHODIMP Web_service_request::get_String_content( BSTR* result )
{
    ...
    try
    {
        hr = Charset::for_name( _http_request->character_encoding() ).Encoded_to_bstr( _http_request->_body, result );
    }
...
STDMETHODIMP Web_service_request::get_Binary_content( SAFEARRAY** result )
{
    ...
    try
    {
        Locked_safearray<unsigned char> safearray ( _http_request->_body.length() );
        memcpy( &safearray[0], _http_request->_body.data(), _http_request->_body.length() );
...
: 
    ...
    _web_service_operation(web_service_operation),
    _http_response( Z_NEW( Http_response( web_service_operation->_web_service_request->_http_response ) ) )
{
}
