...
const int DOPPELTE_KONSTANTEN = 0;
...
#   ifdef Z_WINDOWS
        ...
        Z_LOG2( "scheduler", "FindFirstChangeNotification( \"" << _directory_path << "\", TRUE, FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME );\n" );
        
        HANDLE h = FindFirstChangeNotification( _directory_path.c_str(), 
                                                TRUE,                               // Mit Unterverzeichnissen
                                                FILE_NOTIFY_CHANGE_FILE_NAME  |  
                                                FILE_NOTIFY_CHANGE_DIR_NAME   |
                                                FILE_NOTIFY_CHANGE_LAST_WRITE );

        if( !h  ||  h == INVALID_HANDLE_VALUE )  throw_mswin( "FindFirstChangeNotification", _directory_path );
...
{
    Z_LOGI2( "scheduler", Z_FUNCTION << " Prüfe Konfigurationsverzeichnis " << _directory_path << "\n" );
...
//Absolute_path Directory::make_path( const string& name )
//{
...
bool Directory::read()
{
           ...
           e  != _ordered_list.end() )
    {
               ...
               e->_file_info->path().name() == (*fi)->path().name() )
        {
            ...
            if( e->_file_info->path().name() != (*fi)->path().name()  &&
                e->_file_info->last_write_time() != (*fi)->last_write_time() ) 
            {
                e->_file_info = *fi;
                e->_timestamp = now;
            ...
            if( e->_is_aging  &&  e->_timestamp + file_timestamp_delay < now )
            {
                e->_is_aging = false;
            }
            ...
            if( e->_subdirectory )
            {
                ...
                directory_has_changed = e->_subdirectory->read();
            }
            else
               ...
               ( e == _ordered_list.end()  ||  (*fi)->path().name() < e->_file_info->path().name() ) )
        {
            ...
            new_entry->_timestamp = now;
            
            if( (*fi)->is_directory() )  
            {
                new_entry->_subdirectory = Z_NEW( Directory( _directory_tree, this, e->_file_info->path().name() ) );
                new_entry->_subdirectory->read();
            }
            else
            {
                new_entry->_is_aging = true;
                _directory_tree->set_aging_until( now + file_timestamp_delay );
            }
               ...
               ( fi == ordered_file_infos.end()  ||  (*fi)->path().name() > e->_file_info->path().name() ) )  // Datei entfernt?
        {
            ...
            if( e->_is_removed  &&  e->_timestamp + remove_delay < now )  
            {
            ...
            else  
            {
                ...
                e->_is_removed = true;
                e->_is_aging   = true;
                _directory_tree->set_aging_until( now + remove_delay );
            }
...
bool Directory_tree::read()
{
    ...
    if( _root_directory )  
    {
        directory_has_changed = _root_directory->read();
    }
...
{
    if( _refresh_aged_entries_at  &&  _refresh_aged_entries_at >= double_from_gmtime() )
    {
        bool anything_is_refreshed = _root_directory->refresh_aged_entries();
        if( anything_is_refreshed )  _refresh_aged_entries_at = 0;
    }
}
