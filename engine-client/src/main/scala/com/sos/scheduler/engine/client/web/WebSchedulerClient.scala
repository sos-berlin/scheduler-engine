...
import com.sos.jobscheduler.data.event.{Event, EventId, KeyedEvent, SomeEventRequest, Stamped, TearableEventSeq}
...
trait WebSchedulerClient extends SchedulerClient with WebCommandClient {
  ...
  def fileBaseds[P <: TypedPath: TypedPath.Companion, V <: FileBasedView: FileBasedView.Companion](query: PathQuery): Future[Stamped[Seq[V]]] =
    get[Stamped[Seq[V]]](_.fileBaseds[P](query, returnType = implicitly[FileBasedView.Companion[V]].name))

  def anyTypeFileBaseds[V <: FileBasedView: FileBasedView.Companion](query: PathQuery): Future[Stamped[Seq[V]]] =
    get[Stamped[Seq[V]]](_.anyTypeFileBaseds(query, returnType = implicitly[FileBasedView.Companion[V]].name))
  ...
  final def ordersBy[V <: OrderView: OrderView.Companion](query: OrderQuery): Future[Stamped[Seq[V]]] =
    post[OrderQuery, Stamped[Orders[V]]](_.order.forPost[V], query) map { _ map { _.orders }}

  final def getOrdersBy[V <: OrderView: OrderView.Companion](query: OrderQuery): Future[Stamped[Seq[V]]] =
  ...
  final def orderEvents[E <: Event: ClassTag](query: OrderQuery, after: EventId, timeout: Duration, limit: Int = Int.MaxValue): Future[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]] =
    get[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]](_.order.events(query, after = after, timeout, limit = limit, returnType = implicitClass[E].getSimpleName))
  ...
  final def jobChainOverviewsBy(query: JobChainQuery): Future[Stamped[Seq[JobChainOverview]]] =
    query.pathQuery match {
      ...
      case _ ⇒
        get[Stamped[Seq[JobChainOverview]]](_.jobChain.overviews(query))
    }
  ...
  final def jobChainEvents[E <: Event](jobChainPath: JobChainPath, eventRequest: SomeEventRequest[E]) =
    get[Stamped[TearableEventSeq[Seq, E]]](_.jobChain.events(jobChainPath, eventRequest))
  ...
  final def jobChainEventsBy[E <: Event](query: JobChainQuery, eventRequest: SomeEventRequest[E]) = {
    ...
    get[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]](_.jobChain.events(query, eventRequest))
  }
  ...
  def jobs[V <: JobView: JobView.Companion](query: PathQuery) =
    get[Stamped[Seq[V]]](_.job[V](query))

  def jobEvents[E <: Event](jobPath: JobPath, eventRequest: SomeEventRequest[E]) =
    get[Stamped[TearableEventSeq[Seq, E]]](_.job.events(PathQuery(jobPath), eventRequest))

  def jobEventsBy[E <: Event](query: PathQuery, eventRequest: SomeEventRequest[E]) =
    get[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]](_.job.events(query, eventRequest))

  def taskEvents[E <: Event](taskId: TaskId, eventRequest: SomeEventRequest[E]) =
    get[Stamped[TearableEventSeq[Seq, E]]](_.task.events(taskId, eventRequest))

  def taskEventsBy[E <: Event](query: PathQuery, eventRequest: SomeEventRequest[E]) =
    get[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]](_.task.eventsBy(query, eventRequest))
  ...
  final def processClasses[V <: ProcessClassView: ProcessClassView.Companion](query: PathQuery) =
    get[Stamped[Seq[V]]](_.processClass.views[V](query))
  ...
  final def events[E <: Event](request: SomeEventRequest[E]): Future[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]] =
    get[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]](_.events(request))

  final def eventsByPath[E <: Event](request: SomeEventRequest[E], query: PathQuery): Future[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]] =
    get[Stamped[TearableEventSeq[Seq, KeyedEvent[E]]]](_.eventsByPath(request, query))
