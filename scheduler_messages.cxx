...
Message_code_text scheduler_messages[] =
{
    ...
    { "SCHEDULER-143", "Task would never start because it has no start time and the job's <run_time> has no further period" },
    { "SCHEDULER-145", "<script> com_class= and language= cannot be combined" },
    { "SCHEDULER-146", "Missing script" },
    { "SCHEDULER-147", "Job $1 accepts no orders because it has not been declared with <job order=\"yes\">." },
    ...
    { "SCHEDULER-184", "Scheduler database is currently not accessible due to a database problem" },
    { "SCHEDULER-186", "$1 is already in job chain $2" },
    { "SCHEDULER-187", "Order.setback(): An order can only be set back when it is being executed by a task it" },
    ...
    { "SCHEDULER-208", "Only a date without time is allowed" },
    ...
    { "SCHEDULER-222", "This command <$1> is only executable via a TCP connection" },
    ...
    { "SCHEDULER-226", "Order has not been processed because spooler_init() or spooler_open() returned false" },
    ...
    { "SCHEDULER-271", "The task is being terminated in favour of job $1" },
    ...
    { "SCHEDULER-912", "add_pid($1), deadline for the process is $2" },
    ...
    { "SCHEDULER-945", "End state reached, order has been carried out" },
    { "SCHEDULER-946", "setback(): order has been set back $1 times, until $2" },
    { "SCHEDULER-947", "setback(): order has been set back $1 times. This is more than the job's maximum $2" },
