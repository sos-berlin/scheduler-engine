...
public class JMSEventPlugIn implements PlugIn, EventSubscriber {
	...
	JMSEventPlugIn(Scheduler scheduler, Element plugInElement) {
		...
		// TODO get Hostname and port form JobScheduler instance an initialize
		// the ObjectFactory with them
		logger.info("initializing SchedulerObjectFactory for "
				+ scheduler.getHostname() + ":" + scheduler.getTcpPort());
		objFactory = new SchedulerObjectFactory(scheduler.getHostname(),
				scheduler.getTcpPort());
