...
struct Order_id_space : Object, Scheduler_object
{
  ...
  private:
    ...
    Fill_zero                  _zero_;
    int                        _index;                      // this == Order_id_spaces::_group_array[ index ]
...
struct Order_id_spaces
{
    ...
    void                        remove_order_id_space       ( Order_id_space* );
  ...
  private:
    ...
    vector< ptr<Order_id_space> >  _group_array;           // [0] unbenutzt, Lücken sind NULL
};
...
struct Order_subsystem : Order_subsystem_interface
{
    ...
    Job_chain*                  job_chain                   ( const string& path );
    Job_chain*                  job_chain_or_null           ( const string& path );
    ...
    string                      job_chain_db_where_condition( const string& job_chain_path );
    string                      order_db_where_condition    ( const string& job_chain_path, const string& order_id );
...
void Order_subsystem::add_job_chain( Job_chain* job_chain )
{
    try
    {
        ...
        string lname = lcase( job_chain->path() );
...
Job_chain* Order_subsystem::job_chain_or_null( const string& path )
{
    Job_chain* result = NULL;

    string lname = lcase( path );
...
Job_chain* Order_subsystem::job_chain( const string& path )
{
    Job_chain* result = job_chain_or_null( path );
    if( !result )  z::throw_xc( "SCHEDULER-161", path );
...
ptr<Order> Order_subsystem::try_load_order_from_database( Transaction* outer_transaction, const string& job_chain_path, const Order::Id& order_id, Load_order_flags flag )
{
    ptr<Order> result;
...
string Order_subsystem::order_db_where_condition( const string& job_chain_path, const string& order_id )
{
    S result;
...
string Order_subsystem::job_chain_db_where_condition( const string& job_chain_path )
{
    S result;
...
void Job_chain_node::close()
{
    ...
    if( _job_chain  &&  _nested_job_chain )  
    {
        ...
        _job_chain->order_subsystem()->order_id_spaces()->recompute_job_chain_groups( disconnected_job_chains );
    }
...
//---------------------------------------------------------------Job_chain_node::action_from_string

Job_chain_node::Action Job_chain_node::action_from_string( const string& str )
{
    Action result;
...
//---------------------------------------------------------------Job_chain_node::string_from_action

string Job_chain_node::string_from_action( Action action )
{
    string result;
...
//---------------------------------------------------Job_chain_node::on_releasing_referenced_object

void Job_chain_node::on_releasing_referenced_object( const reference< Job_chain_node, Job_chain >& ref )
{
...
//-------------------------------------------------------------------------Job_chain_node::obj_name

string Job_chain_node::obj_name() const
{
    S result;
    result << "Job_chain_node " << _job_chain->path() << ":" << _state;
    return result;
}
...
void Job_chain::close()
{
    ...
    {
        order_subsystem()->order_id_spaces()->recompute_job_chain_groups( disconnected_job_chains );
    }
    ...
    DOM_FOR_EACH_ELEMENT( element, e )
    {
        ...
        else
        {
            ...
            if( e.nodeName_is( "job_chain_node" ) )
            {
                ...
                string job_chain_path = e.getAttribute( "job_chain" );
                string state          = e.getAttribute( "state" );
...
xml::Element_ptr Job_chain::dom_element( const xml::Document_ptr& document, const Show_what& show )
{
    ...
    //THREAD_LOCK( _lock )
    {
        ...
        if( _order_id_space )  element.setAttribute( "order_id_space", _order_id_space->name() );
...
Job_chain_node* Job_chain::add_job_chain_node( const string& nested_job_chain_path, const Order::State& state, const Order::State& next_state, const Order::State& error_state )
{
...
Job_chain_node* Job_chain::add_node( const Order::State& state, const Order::State& next_state, const Order::State& error_state )
{
    ...
    if( node_from_state_or_null( state ) )  z::throw_xc( "SCHEDULER-150", debug_string_from_variant( state ), path() );
...
void Job_chain::finish()
{
    ...
    if( !_chain.empty() )
    {
        Job_chain_node* node = *_chain.rbegin();
        if( node->_job  &&  node->_next_state.is_missing() )  add_end_node( default_end_state_name );    // Endzustand fehlt? Dann hinzufügen
    }
    ...
    for( Chain::iterator it = _chain.begin(); it != _chain.end(); it++ )
    {
        ...
        if( node->_nested_job_chain_path != "" )
        {
            Job_chain* nested_job_chain = order_subsystem()->job_chain( node->_nested_job_chain_path );    // Sicherstellen, dass Jobkette bekannt ist
            order_id_space->connect_job_chain( nested_job_chain );
        }

        if( node->is_file_order_sink() )
        {
            // _next_state und _error_state unverändert lassen
        }
        else
        {
            if( node->_next_state.is_missing()  &&  next != _chain.end()  &&  ( node->_job || node->_nested_job_chain_path != "" ) )
                node->_next_state = (*next)->_state;

            if( !node->_next_state.is_missing() )  node->_next_node  = node_from_state( node->_next_state );
                                             else  node->_next_state = empty_variant;

            if( !node->_error_state.is_missing() )  node->_error_node  = node_from_state( node->_error_state );
    ...
    for( Chain::iterator it = _chain.begin(); it != _chain.end(); it++ )
    {
        ...
        if( node->_nested_job_chain_path != "" )
        {
            node->_nested_job_chain = order_subsystem()->job_chain( node->_nested_job_chain_path );
        }
    }
    ...
    if( order_id_space->size() > 0 )    // Die verschachtelten Jobketten
    {
        ...
        order_id_space->finish();
        order_subsystem()->order_id_spaces()->add_order_id_space( order_id_space );
        ...
        log()->info( message_string( "SCHEDULER-872", order_id_space->path(), job_chains_string ) );
    }
...
Job_chain_node* Job_chain::node_from_job( Job* job )
{
    ...
    z::throw_xc( "SCHEDULER-152", job->name(), path() );
    return NULL;
}

//------------------------------------------------------------Job_chain::referenced_node_from_state

Job_chain_node* Job_chain::referenced_node_from_state( const Order::State& state )
{
    Job_chain_node* node   = node_from_state( state );
    Job_chain_node* result = node;
    int             n      = 1000;

    while( result->_action == Job_chain_node::act_next_state )
    {
        result = node_from_state( result->_next_state );
        if( --n <= 0 )  z::throw_xc( "SCHEDULER-403", node->_state );
    }
...
void Job_chain::add_order( Order* order )
{
    ...
    Job_chain_node* node = referenced_node_from_state( order->_state );
    if( node != node_from_state( order->_state ) )  
    {
        _log->info( message_string( "SCHEDULER-859", node->_state.as_string(), order->_state ) );
        order->set_state2( node->_state );
    }

    if( ( !order->_suspended || !order->_is_on_blacklist )  &&  !node->_job )  z::throw_xc( "SCHEDULER-149", path(), debug_string_from_variant(order->_state) );
...
{
    assert( order->_job_chain_path == _name );
...
{
    if( is_referenced() )  z::throw_xc( "SCHEDULER-425", obj_name(), is_referenced_by<Job_chain_node,Job_chain>::string_referenced_by() );
...
//------------------------------------------------------Order_id_spaces::recompute_job_chain_groups
    
void Order_id_spaces::recompute_job_chain_groups( const Job_chain_set& disconnected_job_chains )
{
    if( !disconnected_job_chains.empty() )
    {
        Job_chain_set   job_chains          = disconnected_job_chains;
        Order_id_space* old_job_chain_group = ( *disconnected_job_chains.begin() ) -> order_id_space();
        ...
        while( !job_chains.empty() )
        {
            ...
            {
                if( connected_job_chains == job_chain->order_id_space()->_job_chain_set )  break;  // Nur beim ersten Schleifendurchlauf, gilt für alle Jobketten

                remove_order_id_space( old_job_chain_group );
            ...
            else
            {
                ...
                order_id_space->finish();
                add_order_id_space( order_id_space, new_group_index );
                new_group_index = 0;
            }
        }
    }
...
//-----------------------------------------------------------Order_id_spaces::disconnect_job_chains

//void Order_id_spaces::disconnect_job_chains( Job_chain* a, Job_chain* b )
//{
//    Order_id_space* a_group = a->order_id_space();
//
//    assert( a_group  &&  a_group == b->order_id_space() );
//
//
...
//    {
//        ptr<Order_id_space> b_group = Z_NEW( Order_id_space( _order_subsystem->_spooler ) );
//
...
//                jc->set_order_id_space( NULL );                
//            }
//
//            remove_order_id_space( a_group );
//        }
//                
//        if( b_group->size() <= 1 )
//        {
...
void Order_id_spaces::add_order_id_space( Order_id_space* order_id_space, int index )
{
    ...
    if( index == _group_array.size() )  _group_array.push_back( order_id_space );
                                  else  _group_array[ index ] = order_id_space;
...
void Order_id_spaces::remove_order_id_space( Order_id_space* order_id_space )
{
    assert( _group_array[ order_id_space->_index ] == order_id_space );

    order_id_space->log()->info( message_string( "SCHEDULER-873" ) );
...
void Order_id_space::connect_job_chain( Job_chain* job_chain )
{
    ...
    //Job_chain_set job_chain_set = job_chain->connected_job_chains();

    //Z_FOR_EACH( Job_chain_set, job_chain_set, it )
    //{
    //    Job_chain* jc = *it;

    //    if( _job_chain_set.find( jc ) != _job_chain_set.end() )
    //    {
    //        add_job_chain( jc );    // Exception, wenn doppelte Auftragskennung
    //    }
    //}
}
...
void Order_id_space::finish()
{
    //stdext::hash_set<Order_id_space*> disconnected_job_chain_groups;
    ...
    Z_FOR_EACH( Job_chain_set, _job_chain_set, it )
    {
        ...
        Order_id_space* previous_job_chain_group = job_chain->order_id_space();
        if( previous_job_chain_group != this )
        {
            if( previous_job_chain_group )
            {
                ...
                {
                    order_subsystem()->order_id_spaces()->remove_order_id_space( previous_job_chain_group );
                }
                else
...
//        Job_chain* job_chain = *it;
//        
//        if( Order_id_space* old_job_chain_group = job_chain->order_id_space() )
//        {
...
//            {
//                order_subsystem()->order_id_spaces()->remove_order_id_space( old_job_chain_group );
//            }
//        }
//
//        job_chain->set_order_id_space( this );
...
void Order_id_space::check_for_unique_order_ids_of( Job_chain* job_chain ) const
{
    Z_FOR_EACH( Job_chain::Order_map, job_chain->_order_map, it )
    {
        if( Job_chain* other_job_chain = job_chain_by_order_id_or_null( it->first ) )
            z::throw_xc( "SCHEDULER-426", it->first, job_chain, other_job_chain );
    }
}
...
string Order_id_space::name() const
{
    ...
    switch( _index )
    {
        ...
        case 1:     result << "strawberry";     break;
        case 2:     result << "kiwi";           break;
        case 3:     result << "orange";         break;
        case 4:     result << "raspberry";      break;
        case 5:     result << "pear";           break;
        case 6:     result << "apricot";        break;
        case 7:     result << "peach";          break;
...
Job_chain_set Job_chain::connected_job_chains()
{
    Job_chain_set result;
...
void Job_chain::get_connected_job_chains( Job_chain_set* result )
{
    ...
    {
        Job_chain_node* node      = (*it)->referer();
        Job_chain*      job_chain = node->_job_chain;

        if( result->find( job_chain ) == result->end() )  
        {
            result->insert( job_chain );
...
string Order_queue::db_where_expression_for_job_chain( const Job_chain* job_chain )
{
    ...
    result << "(`job_chain`="  << sql::quoted( job_chain->path() ) << " and `state` in (";
...
bool Order::db_try_insert()
{
    ...
    for( Retry_transaction ta ( _spooler->_db ); ta.enter_loop(); ta++ ) try
    {
        ...
        insert[ "job_chain"     ] = _job_chain_path;
...
void Order::db_fill_where_clause( sql::Where_clause* where )
{
    ...
    where->and_where_condition( "job_chain" , _job_chain_path       );
...
void Order::set_state1( const State& state )
{
    ...
    if( _job_chain )
    {
        ...
        if( previous_state != _state  &&  ( !_job_chain_node  ||  !_job_chain_node->_job ) )
        {
            handle_end_state();
        }
    }
...
void Order::add_to_order_queue( Order_queue* order_queue )
{
    ...
    //THREAD_LOCK( _lock )
    {
        ...
        if( _job_chain_path != "" )  remove_from_job_chain();
    ...
    //THREAD_LOCK( _lock )
    {
        ...
        if( !is_setback()  &&  !_moved  &&  !_end_state_reached  ||  force_error_state )
        {
            if( _job_chain_node )
            {
                ...
                set_state1( success? _job_chain_node->_next_state
                                   : _job_chain_node->_error_state );

                //set_state1():  Kein clear_setback();
