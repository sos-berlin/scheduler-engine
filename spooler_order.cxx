...
Job_node::Job_node( Job_chain* job_chain, const Order::State& state, const string& job_path ) 
: 
...
void Job_chain::remove_order( Order* order )
{
    ...
    if( order->_task )
    {
        order->_removed_from_job_chain_name = name();      // Für die Task merken, in welcher Jobkette wir waren
        order->_moved = true;
    }
        ...
        int count = ta->open_result_set
            ( 
                S() << "select count(*)  from " << _spooler->_orders_tablename << 
                       "  where " << order_subsystem()->order_db_where_condition( name(), order_id.as_string() ),
                __FUNCTION__
            )
            .get_record().as_int( 0 );
...
{ 
    return order_subsystem()->job_chain_db_where_condition( name() ); 
}
...
void Order_id_spaces::recompute_order_id_spaces( const Job_chain_set& disconnected_job_chains, Job_chain* causing_job_chain )
{
    ...
    while( !job_chains.empty() )
    {
        ...
        else
        {
            ...
            {
                Job_chain* jc = _order_subsystem->job_chain( *it2 );
...
void Order_id_space::close()
{
    ...
    {
        if( Job_chain* job_chain = spooler()->order_subsystem()->job_chain_or_null( *it ) )
        {
            job_chain->set_order_id_space( NULL );
        }
    }
...
void Order_id_space::on_order_id_space_added( Job_chain* causing_job_chain )
{
    ...
    Z_FOR_EACH( String_set, _job_chain_set, it )
    {
        Job_chain* job_chain = spooler()->order_subsystem()->job_chain( *it );
        if( job_chain != causing_job_chain )
        {
            if( !job_chains_string.empty() )  job_chains_string << ", ";
            job_chains_string << '\'' << job_chain->path() << '\'';
...
void Order_id_space::connect_job_chain( Job_chain* job_chain )
{
    ...
    if( Order_id_space* order_id_space = job_chain->order_id_space() )
    {
        ...
        {
            Job_chain* jc = spooler()->order_subsystem()->job_chain( *it );
            add_job_chain( jc );
...
void Order_id_space::add_job_chain( Job_chain* job_chain, bool check_ids )
{
    ...
    _job_chain_set.insert( job_chain->path() );
}
...
void Order_id_space::remove_job_chain( Job_chain* job_chain )
{
    ...
    job_chain->set_order_id_space( NULL );
    _job_chain_set.erase( job_chain->path() );
}
...
{ 
    ...
    {
        Z_LOG2( "scheduler.order", obj_name() << "  " << __FUNCTION__ << "(" << t << ( is_now? ",is_now" : "" ) << ")  vorher: " << _next_announced_distributed_order_time << "\n" );
    }
    ...
    if( is_now )
    {
        ...
        if( Job_node* job_node = Job_node::try_cast( _order_queue_node ) )
            if( Job* job = job_node->job_or_null() )  job->signal( __FUNCTION__ );
    }
}
...
{
    Z_LOG2( "scheduler.order", obj_name() << "  " << __FUNCTION__ << "\n" );
...
{
    S  result;
    result << "`job_chain`="  << sql::quoted( _job_chain->path() ) 
