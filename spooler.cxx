    ...
    THREAD_LOCK( _lock )
    {
        ...
        while( it != _job_list.end() )
        {
            ...
            if( !which_job  ||  which_job == job )
            {
                ...
                {
                    job->_log->log( job->temporary()? log_debug : log_info, "Job wird jetzt entfernt" );
...
*/
  ...
  //_spooler_thread_list.clear();
    ...
    {
        sleep( 2.0 );   // Etwas warten, damit der Browser nicht hängenbleibt. Sonst wird die HTTP-Verbindung nicht richtig geschlossen. Warum bloß?
