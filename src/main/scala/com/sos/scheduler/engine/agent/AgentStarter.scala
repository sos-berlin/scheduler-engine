...
final class AgentStarter @Inject private(
  ...
  private val webServiceActorRef = actorSystem actorOf Props(new AgentWebServiceActor(executeCommand = commandExecutor.execute))
  private implicit val timeout: Timeout = 10.seconds

  def start(): Future[Unit] =
    IO(Http) ? Http.Bind(webServiceActorRef, interface = conf.httpInterfaceRestriction getOrElse "", port = conf.httpPort) flatMap {
      ...
      case Tcp.CommandFailed(_: Http.Bind) ⇒
        ...
        Future.failed(new RuntimeException(s"Binding to TCP port ${conf.httpPort} failed. Port is possibly in use and not available. Switch on DEBUG-level logging for `akka.io.TcpListener` to log the cause"))
    }
