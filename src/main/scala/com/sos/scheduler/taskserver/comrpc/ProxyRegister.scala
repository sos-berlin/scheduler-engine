...
import com.sos.scheduler.taskserver.comrpc.ProxyRegister._
 ...
 */
@Singleton
final class ProxyRegister @Inject private() {
  private val proxyIdToProxy = mutable.Map[ProxyId, Entry]()
  private val iunknownToProxyId = mutable.Map[IUnknown, ProxyId]()
  private val proxyIdIterator = Iterator from 1 map { i: Int ⇒ ProxyId((Random.nextInt().toLong << 32) + i) }  // FIXME Eindeutig zu Proxy-IDs des Gegenübers
