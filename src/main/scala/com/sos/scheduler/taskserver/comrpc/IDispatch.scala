...
import com.sos.scheduler.taskserver.comrpc.IDispatch._
import com.sos.scheduler.taskserver.comrpc.types.{DISPID, DispatchType, EmptyVariant, VariantArray}
...
trait IDispatch extends IUnknown {
  ...
  private def methodIndex(name: String): Int = {
    ...
    if (methodIndices.size > 1) throw new RuntimeException(s"Ambigous method name '$name'")
    methodIndices.head
  }
